...
const AC_TYPE_NAMES: Record<string, string> = {
  total: 'AC',
  touch: 'Touch',
  flatFooted: 'FF',
}

type MiniACCardProps = {
  label: string
  value: number
  onPress?: () => void
  glowEnabled?: boolean
...
const MiniACCard: React.FC<MiniACCardProps> = ({ label, value, onPress, glowEnabled = true }) => {
  ...
  return (
    <Animated.View
      style={[styles.miniCardWrapper, animatedBorderStyle, { backgroundColor: colors.background }]}
    >
      {/* Background glow */}
...
export type ArmorClassCardProps = {
  totalAC: number
  touchAC: number
  flatFootedAC: number
  onPress?: () => void
...
/**
 * Card de Armor Class con 3 mini-cards horizontales.
 * Layout: [AC 22] [Touch 14] [FF 18]
 * Cada mini-card tiene animación de brillo cuando su valor cambia.
 */
export const ArmorClassCard: React.FC<ArmorClassCardProps> = ({
  totalAC,
  touchAC,
  flatFootedAC,
  onPress,
