...
**Prerequisitos:** Fase 5 completada (o al menos Fase 4, ya que Tauri es paralelo)

---

## ⚠️ Principio Fundamental: Solo Componentes Grandes

**NO migraremos átomos.** Los átomos (Button, Input, Select, Checkbox, etc.) se crearán de nuevo basándose en zukus-again.

**SÍ migraremos:**
- Componentes con lógica de estado compleja
- Hooks que encapsulan lógica de negocio
- Sistemas completos (navegación, formularios de Changes, EntityProvider)
...
| `useNavigationContext` | `src/components/` | Sistema de navegación (410+ líneas) |
...
### 6.2 Migrar useNavigationContext

Este es un hook complejo (~410 líneas) que maneja:
- Stack de paneles
- Navegación en desktop (panel lateral)
- Navegación en mobile (modals/stacks)

```
📁 Crear packages/ui/src/hooks/useNavigationContext.ts
```

**Consideraciones de adaptación:**
- Eliminar dependencias de `useRouter` de Next.js
- Parametrizar la sincronización con URL (no hardcodear)
- Mantener la lógica del stack de paneles
...
```bash
git add -A
git commit -m "feat(ui): migrate useNavigationContext from zukusnextmicon"
```

---

### 6.3 Migrar EntityProvider
...
### 6.4 Migrar componentes de Changes
...
### 6.5 Actualizar exports principales
...
### 6.6 Verificar uso desde mobile
...
- [ ] `useNavigationContext` está migrado y adaptado
